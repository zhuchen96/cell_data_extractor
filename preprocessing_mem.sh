...
# Define output path
OUTPUT_MASK_PATH="$(pwd)/zarr_masks"
OUTPUT_IMG_PATH="$(pwd)/zarr_images"
OUTPUT_JSON_PATH="$(pwd)/json_files"
...
python3 process_mask_mem.py --in_path "$INPUT_MASK_PATH" --out_path "$OUTPUT_MASK_PATH" --out_json_path "$OUTPUT_JSON_PATH"
