...
def main(args):
    ...
    for filename in filelist:  # or all files
        if filename.endswith(".tif"):
            ...
            # 2. Build adjacency (direct neighbors)
            adjacency = build_adjacency_count(data)
            ...
            for label_value in unique_labels:
                ...
                # neighbor detection
                # adjacency gives direct neighbors; if you want "within 10 voxels",
                # do morphological approach or bounding-box filters here
                neighbors = adjacency.get(int(label_value), [])
                ...
                # changed neighbors from previous iteration
                changed_neighbors = None
                if label_value in results and tf_num-1 in results[label_value]:
                    prev_neighbors = results[label_value][tf_num-1].get("neighboring_cell_ids", [])
                    #print(set(neighbors))
                    #print(set(prev_neighbors))
                    changed_neighbors = len(set(neighbors).symmetric_difference(set(prev_neighbors)))
                    #print(changed_neighbors)
                ...
                results[label_value][tf_num] = {
                    ...
                    "centroid": centroid,
                    "movement": movement,
                    "neighboring_cell_ids": neighbors,
